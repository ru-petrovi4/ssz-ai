...
public partial class Model10View : UserControl
{
    ...
    {
        var lastAddedMemory = (_model.Temp_ActivitiyMaxInfo?.SelectedSuperActivityMax_MiniColumn as Cortex_Simplified.MiniColumn)?.Temp_Memory;
        if (lastAddedMemory is not null)
            ((Cortex_Simplified.MiniColumn)_model.Temp_ActivitiyMaxInfo!.SelectedSuperActivityMax_MiniColumn!).Memories.Remove(lastAddedMemory);
