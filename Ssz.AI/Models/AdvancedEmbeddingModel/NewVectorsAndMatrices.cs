...
{
    public class NewVectorsAndMatrices
    {
        /// <summary>
        ...
        public void Initialize(int wordsCount)
        {
            ...
            {
                NewVectors[wordIndex] = new float[Model.NewVectorLength];
            }

            NewVectors_PrimaryOnly = new float[wordsCount][];            
            foreach (int wordIndex in Enumerable.Range(0, wordsCount))
            {
                NewVectors_PrimaryOnly[wordIndex] = new float[Model.NewVectorLength];
            }

            NewVectors_SecondaryOnly = new float[wordsCount][];            
            foreach (int wordIndex in Enumerable.Range(0, wordsCount))
            {
                NewVectors_SecondaryOnly[wordIndex] = new float[Model.NewVectorLength];
            }
        ...
        public void InitializeTemp(Clusterization_Algorithm clusterization_Algorithm, List<Word> words, float[] proxWordsOldMatrix)
        {
            ...
            foreach (int clusterIndex in Enumerable.Range(0, Temp_ClusterWords.Length))
            {
                ...
                foreach (Word word in clusterWords)
                {
                    ...
                    var temp_Top8ProxWords = clusterWords.Select(w => (proxWordsOldMatrix[indexBias + w.Index], w))
                    ...
                    var temp_Top8ProxPrimaryWords = primaryWords.Select(w => (proxWordsOldMatrix[indexBias + w.Index], w))
                        .OrderByDescending(it => it.Item1)
                        .Take(8)
                        .ToArray();
            ...
            for (int rangeIndex = count - 1; rangeIndex >= 0; rangeIndex -= 1)
            {
                ...
                var pairGroup = proxWordsOldMatrix.Select((f, i) => (f, i))
