...
{    
    ...
    private void CalculateActivityAndSuperActivity(float[] discreteVector, ActivitiyMaxInfo? activitiyMaxInfo)
    {
        Parallel.For(
            ...
            mci =>
            {
                ...
                mc.Temp_Activity = MiniColumnsActivityHelper.GetActivity(mc, discreteVector, Constants);
            });
    ...
    private void ClearActivityAndSuperActivity(ActivitiyMaxInfo? activitiyMaxInfo)
    {
        Parallel.For(
            fromInclusive: 0,
            toExclusive: MiniColumns.Data.Length,
